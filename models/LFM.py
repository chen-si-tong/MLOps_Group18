...
class SVDModel(tf.keras.Model):
    ...
    def __init__(self, cfg):
        ...
        self.user_emb = tf.keras.layers.Embedding(
            cfg["user_num"], cfg["dim"], embeddings_initializer=initializer, trainable=True
        )
        self.item_emb = tf.keras.layers.Embedding(
            cfg["item_num"], cfg["dim"], embeddings_initializer=initializer, trainable=True
        )
    ...
    def call(self, inputs):
        ...
        output_star = tf.clip_by_value(
            output, 1.0, 5.0
        )  # Clip scores, setting them to 0 if they are less than 0 and to 5 if they are greater than 5.
