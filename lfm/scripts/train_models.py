...
def _main(cfg: DictConfig, train_step=train_step, validation_step=validation_step):
    ...
    train_data = DataSampler([df_train["user"], df_train["item"], df_train["rate"]], batch_size=enc.epoch["batch_size"])
    validation_data = DataSampler(
        [df_validation["user"], df_validation["item"], df_validation["rate"]], batch_size=enc.epoch["batch_size"]
    )
...
def train_validaiton(cfg, train_data, validation_data, model, train_step, validation_step):
    ...
    for epoch in range(cfg.num_epochs):
        ...
        if epoch % draw_num == 0:
            ...
            logger.info(
                f"Epoch {epoch+1}: Train Loss = {np.mean(train_loss_batch)}, Train MAE = {np.mean(train_mae_batch)},Time Consuming = {round((end - start),3)}(s)"
            )
            logger.info(
                f"Epoch {epoch+1}: Validation Loss = {np.mean(valid_loss_batch)}, Validation MAE = {np.mean(valid_mae_batch)},Time Consuming = {round((end - start),3)}(s)"
            )
